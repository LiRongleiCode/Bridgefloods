...
$settings = [ordered]@{
  ...
  appearanceLightChromeTheme = 'appearanceLightChromeTheme = { accent = "#475569", contrast = 52, fonts = { code = "Cascadia Code", ui = "Microsoft YaHei UI" }, ink = "#1F2937", opaqueWindows = false, semanticColors = { diffAdded = "#166534", diffRemoved = "#991B1B", skill = "#475569" }, surface = "#F8FAFC" }'
}
...
if (-not $NoShortcuts) {
  ...
  $customizeScript = Join-Path $PSScriptRoot 'customize-theme.ps1'
  foreach ($folder in @($desktop, $startMenu)) {
    ...
    $shortcut.Description = 'Launch Codex with the adaptive image-based Dream Skin'
    $shortcut.Save()
  }
  ...
  $restore.Save()
  $customize = $shell.CreateShortcut((Join-Path $desktop 'Codex Dream Skin - 换图.lnk'))
  $customize.TargetPath = $powershell
  $customize.Arguments = "-NoProfile -ExecutionPolicy Bypass -File `"$customizeScript`" -Port $Port"
  $customize.WorkingDirectory = $SkillRoot
  $customize.Description = 'Choose a personal image and apply it to Codex Dream Skin'
  $customize.Save()
}
