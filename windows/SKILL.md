...
description: Apply, launch, verify, repair, update, or restore an adaptive image-based skin for the Windows Codex desktop app. Use when the user asks for a Codex theme that goes beyond official color settings, wants a neutral translucent interface driven by a selected background image, needs the skin reapplied after a Codex update, or needs a safe rollback without modifying WindowsApps or app.asar.
---
...
1. Run `scripts/install-dream-skin.ps1` once to create launch/restore shortcuts and the user theme directory.
...
- Do not use the full reference screenshot as a fake whole-window overlay. The selected image is a real CSS background, while all controls remain live Codex controls.
- Keep the selected image behind neutral translucent surfaces and use a readable contrast veil over the home hero. Keep the cards below it as native Codex suggestion buttons with native labels/icons.
