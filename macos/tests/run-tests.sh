...
if /usr/bin/grep -R -n -F 'continuing with soft verification' "$ROOT/scripts" >/dev/null; then
  printf 'A runtime script still allows HTTP-only CDP verification.\n' >&2
  exit 1
fi
/usr/bin/grep -q 'verified_cdp_endpoint "$port" || return 1' "$ROOT/scripts/common-macos.sh"
for file in "$ROOT/scripts/switch-theme-macos.sh" "$ROOT/scripts/load-image-theme-macos.sh"; do
  /usr/bin/grep -q 'system attribute "DREAM_SKIN_NOTIFICATION"' "$file"
done
if /usr/bin/grep -R -n -E '/usr/bin/osascript -e "display notification.*\$' "$ROOT/scripts" >/dev/null; then
  printf 'A notification still interpolates user text into AppleScript source.\n' >&2
  exit 1
fi
...
"$NODE" -e '
  ...
  Object.assign(theme, { promoTitle: "Sponsor", promoSub: "example.com", promoUrl: "https://example.com" });
...
"$NODE" -e '
  const value = JSON.parse(process.argv[1]);
  if (!value.payloadIncludesPromo) process.exit(1);
' "$PAYLOAD_JSON"
