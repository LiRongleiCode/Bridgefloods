...
- CDP 注入现在必须同时通过 loopback HTTP 检查和 Codex 进程归属检查，不再接受仅能响应 HTTP 的本机端口
- 主题切换与图片加载通知改为通过环境变量向 AppleScript 传值，避免主题名或文件名中的引号被当作脚本执行

### 改进

- 热重应用与首次启动统一使用严格的 CDP 身份校验，确保注入目标属于官方 Codex 进程树
